...
const baseSize = (Math.floor((window.innerWidth > window.innerHeight ? window.innerWidth : window.innerHeight) / 50) + 1) * 50;
...
export const drawBg = (targetCtx, rx, ry) => {
    ...
    targetCtx.drawImage(bgCanvas, rxo - rxi, ryo, bgCanvas.width, bgCanvas.height);
    targetCtx.drawImage(bgCanvas, rxo, ryo - ryi, bgCanvas.width, bgCanvas.height);
    targetCtx.drawImage(bgCanvas, rxo - rxi, ryo - ryi, bgCanvas.width, bgCanvas.height);
}
